...
const PrivacySettings = () => {
  ...
  const handleContactSyncToggle = async () => {
    try {
      ...
      Alert.alert('Success', `Contact Sync is now ${updatedValue ? 'enabled' : 'disabled'}`);
    } catch (error) {
      console.error('Error updating contact sync:', error);
      Alert.alert('Error', 'There was an issue updating your contact sync settings. Please try again.');
    }
  };

  const handlePrivacyChange = async (newPrivacySetting) => {
    try {
      const user = await getCurrentUser();
      const { error } = await supabase
        .from('profiles')
      ...
      Alert.alert('Success', `Your privacy setting is now ${newPrivacySetting.replace('_', ' ')}`);
    } catch (error) {
      console.error('Error updating privacy setting:', error);
      Alert.alert('Error', 'There was an issue updating your privacy setting. Please try again.');
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.settingContainer}>
        ...
        <Text style={styles.settingDescription}>
          Contact syncing allows you to find friends on the app through your contacts.
        </Text>
      </View>
      ...
      <View style={styles.privacyContainer}>
        ...
        <TouchableOpacity
          style={[
            ...
            privacySetting === 'friends_of_friends' && styles.selectedPrivacyOption,
          ]}
